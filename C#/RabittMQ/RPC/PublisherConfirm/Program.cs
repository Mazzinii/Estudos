...
async Task HandlePublishConfirmsAsynchronously()
{
    ...
    channel.BasicReturnAsync += (sender, ea) =>
    {
        ...
        if (props.Headers is not null)
        {
            ...
            if (maybeSeqNum is byte[] byteArray)
            {
                sequenceNumber = BinaryPrimitives.ReadUInt64BigEndian(byteArray);
            }
            else if (maybeSeqNum is long longValue)
            {
                sequenceNumber = (ulong)longValue;
            }
            else
            {
                Console.WriteLine("Unexpected type for sequence number: " + maybeSeqNum.GetType());
            }
